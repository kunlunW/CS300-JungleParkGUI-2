 ...
 * ParkGUI
 * 
 * Animal is the super class for any animal type in our JunglePark application It defines the common
 * features for all these animals as well as a default behavior. The class Animal implements also
 * the ParkGUI interface.
 * 
 * About PApplet: A Processing window is a special type of Java program called a PApplet. This is
 * technically a Java Class, which has its own main function (or method) and does all kinds of fancy
 * stuff that we don't need to worry about to create a new window and draw graphics onto it. In
 * order for us to make use of the PApplet class, we want our program to BE a PApplet. To make this
 * happen, we use the keyword extends in our class declaration. This will allow our class to
 * 'inherit' all the PApplet class' functions and variables.
 * 
...
public class Animal implements ParkGUI {
   ...
   * 
   * It is worth to note that action() method as defined in the Animal class does not specify any
   * particular behavior for this animal. The behavior of each animal will depend on the information
   * about the area around that animal (presence of food or threat for instance). Therefore, this
   * method should be overridden by the subclasses.
   */
  public void action() {
  ...
  /**
   * Check if otherAnimal such as a tiger is located within the range distance (alerting distance)
   * of the current animal
   * 
   * @param otherAnimal a reference to another animal (could be either tiger or deer)
   * @param range the range which an animal could sense the presence of another neighboring animal
   * @return animalNearby true if an animal is within the range, false otherwise
   * 
   */
